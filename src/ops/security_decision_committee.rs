...
// 2026-04-09 CST: 这里定义正式投委会请求合同，原因是 Task 1 的 chair 需要一个稳定的 committee 输入对象，
// 目的：把标的、环境和风险参数统一收口，支持单次调用完成“证据 -> 委员会 -> 决策卡”流程。
...
// 2026-04-09 CST: 这里定义正式投委会结果，原因是 chair / scorecard / package 后续都要读取 committee 对象，
// 目的：把证据包、席位意见、票型、风控否决和决策卡固定在一个正式输出里。
...
// 2026-04-09 CST: 这里定义单席 agent 请求，原因是 committee 现在通过子进程执行来保证席位意见独立，
// 目的：让席位内部调用也沿正式 Tool 合同流转，而不是绕开主 dispatcher。
#[derive(Debug, Clone, PartialEq, Deserialize, Serialize)]
