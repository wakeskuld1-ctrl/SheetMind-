...
pub enum StockAnalysisDataGuardError {
    ...
    #[error("股票 `{symbol}` 在请求日期 `{requested_as_of_date}` 之前没有可用历史数据；补数结果：{message}")]
    NoUsableTradeDate {
        symbol: String,
...
pub fn ensure_analysis_date_guard(
    ...
    let exact_local_date = store.latest_trade_date_on_or_before(&request.symbol, &requested_as_of_date)?;
    ...
    if !has_exact_requested_date {
        sync_attempted = true;
        sync_result = Some(run_sync_attempt(store, request, &requested_as_of_date, local_data_last_date.as_deref())?);
    }
    ...
    let (data_freshness_status, date_fallback_reason) = if effective_trade_date == requested_as_of_date {
        if sync_attempted {
...
fn run_sync_attempt(
    ...
    let start_date = derive_sync_start_date(request.lookback_days, requested_as_of_date, local_data_last_date)?;
...
fn parse_date(value: &str) -> Result<NaiveDate, StockAnalysisDataGuardError> {
    NaiveDate::parse_from_str(value, "%Y-%m-%d")
        .map_err(|_| StockAnalysisDataGuardError::InvalidRequestedDate {
