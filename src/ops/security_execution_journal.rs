...
pub fn build_security_execution_journal(
    ...
    if buy_qty <= 0.0 || sell_qty <= 0.0 {
        return Err(SecurityExecutionJournalError::Build(
            "execution_trades must include at least one buy and one sell".to_string(),
        ));
    }
    if (buy_qty - sell_qty).abs() > 1e-9 {
        return Err(SecurityExecutionJournalError::Build(
            "execution_trades must end with fully matched buy/sell position".to_string(),
        ));
    }
    if resulting_position_pct.abs() > 1e-9 {
        return Err(SecurityExecutionJournalError::Build(
            "execution_trades must end flat for current execution journal v1".to_string(),
        ));
    }
    ...
    let weighted_exit_price = sell_notional / sell_qty;
