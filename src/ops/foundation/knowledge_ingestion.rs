...
use crate::ops::foundation::knowledge_record::{
...
// 2026-04-11 CST: 这里补回历史文件，原因是当前仓库仍公开导出该模块且已有测试直接依赖。
// 目的：先恢复既有 foundation 能力，避免证券主链开发被无关缺口卡住。
        ...
        match record {
            ...
            KnowledgeBundleJsonlRecord::Node {
                ...
                nodes.push(KnowledgeNode {
                    ...
                    body,
                    concept_ids,
                    // 2026-04-11 CST: 这里把历史 JSONL string metadata 映射到现行 enum 容器，
                    // 原因是 foundation 已把节点 metadata 升级为通用值类型，旧导入格式仍需兼容。
                    // 目的：只修补类型漂移，不改变既有 JSONL 外部契约。
                    metadata: metadata
                        .into_iter()
                        .map(|(key, value)| (key, MetadataFieldValue::Text(value)))
                        .collect(),
                    evidence_refs,
                });
            }
