...
pub struct MetadataSchema {
    ...
    // 2026-04-10 CST: 这里把 alias 索引正式保存在 schema 内部，原因是 validator 联动阶段已经需要稳定解析
    // “输入字段 key 到 canonical 字段”的映射，不能继续只在构建期用局部变量做一次性校验。
    // 目的：为节点级 validator 提供只读 alias 解析能力，而不把逻辑扩成 rewrite executor。
...
impl MetadataSchema {
    ...
    // 2026-04-10 CST: 这里补输入字段到 canonical 字段定义的统一解析入口，原因是 validator 联动不能再把 alias
    // 当成未知字段或直接用原始 key 做全部校验。
    // 目的：让 direct key 与 alias key 都能收敛到同一份字段定义，而不引入自动改写。
    pub fn field_definition_for_input(&self, field_key: &str) -> Option<&MetadataFieldDefinition> {
        self.field_index
            .get(field_key)
            .or_else(|| self.alias_index.get(field_key))
            .and_then(|index| self.fields.get(*index))
    }

    // 2026-04-10 CST: 这里补 alias -> canonical 字段 key 的解析入口，原因是 validator 需要知道
    // “当前输入是不是 alias，以及 alias 对应哪个正式字段”。
    // 目的：把 alias 识别职责留在 schema 治理层，避免 validator 自己扫描全部字段。
    pub fn canonical_field_key(&self, field_key: &str) -> Option<String> {
        if self.field_index.contains_key(field_key) {
    ...
    // 2026-04-10 CST: 这里补 alias 命中判断，原因是 validator 不只要拿到 canonical 字段，
    // 还要显式区分“这是 direct key 命中”还是“这是 alias 命中”。
    // 目的：为 AliasFieldUsed 这类结构化 issue 提供稳定判断来源。
    pub fn alias_target_key(&self, field_key: &str) -> Option<String> {
        self.alias_index
            .get(field_key)
            .and_then(|index| self.fields.get(*index))
            .map(|field| field.key.clone())
