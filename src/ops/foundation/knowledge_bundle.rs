...
// 2026-04-08 CST: 这里定义标准知识包，原因是 foundation 第二阶段需要先把“什么是可持久化知识单元”
// 固化为通用结构，而不是直接把内存 store 当作落盘格式。
// 目的：为持久化、导入导出和跨域适配提供统一基础载体。
// 2026-04-11 CST: 这里补回缺失文件，原因是当前 `src/ops/foundation.rs` 仍公开导出该模块，
// 但工作区实体文件已缺失，导致任何新的 cargo 编译都会先被无关构建问题阻断。
// 目的：恢复既有 foundation 公共合同，让证券主链的新测试可以继续按 TDD 推进，而不是被外部缺口卡住。
