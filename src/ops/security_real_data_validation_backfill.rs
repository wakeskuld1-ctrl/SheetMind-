...
use crate::ops::stock::security_analysis_fullstack::{
    DisclosureAnnouncement, SecurityAnalysisFullstackError, SecurityAnalysisFullstackRequest,
    fetch_live_disclosure_context_for_governed_history,
    fetch_live_fundamental_context_for_governed_history, security_analysis_fullstack,
};
...
pub fn security_real_data_validation_backfill(
    ...
    if let Ok(fundamental_context) =
        fetch_live_fundamental_context_for_governed_history(&request.symbol)
    {
    ...
    if let Ok(disclosure_context) = fetch_live_disclosure_context_for_governed_history(
        &request.symbol,
        ...
        persist_slice_disclosure_history(
            ...
            request,
            &disclosure_context.recent_announcements,
        )?;
    }
...
fn persist_slice_fundamental_history(
    db_path: &Path,
    request: &SecurityRealDataValidationBackfillRequest,
    context: &crate::ops::stock::security_analysis_fullstack::FundamentalContext,
) -> Result<(), SecurityRealDataValidationBackfillError> {
    ...
    let input = SecurityFundamentalHistoryBackfillRecordInput {
        ...
        report_period: context
            .latest_report_period
            .clone()
            .unwrap_or_else(|| request.end_date.trim().to_string()),
        notice_date: context.report_notice_date.clone(),
        source: context.source.clone(),
        report_metrics: context.report_metrics.clone(),
    };
    let row = SecurityFundamentalHistoryRecordRow {
        symbol: input.symbol,
        report_period: input.report_period,
        notice_date: input.notice_date,
        source: input.source,
        report_metrics_json: serde_json::to_string(&input.report_metrics).map_err(|error| {
            SecurityRealDataValidationBackfillError::FundamentalStorage(format!(
                "failed to serialize slice-local report metrics: {error}"
            ))
        })?,
        batch_id: format!("validation-slice:{}", request.slice_id.trim()),
        record_ref: format!(
    ...
    store.upsert_rows(&[row]).map_err(|error| {
...
fn persist_slice_disclosure_history(
    db_path: &Path,
    request: &SecurityRealDataValidationBackfillRequest,
    announcements: &[DisclosureAnnouncement],
) -> Result<(), SecurityRealDataValidationBackfillError> {
