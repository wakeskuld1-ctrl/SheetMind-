...
impl SecurityResonanceStore {
    ...
    pub fn workspace_default() -> Result<Self, SecurityResonanceStoreError> {
        if let Ok(path) = std::env::var("EXCEL_SKILL_RESONANCE_DB") {
            return Ok(Self::new(PathBuf::from(path)));
        }

        let runtime_dir =
            workspace_runtime_dir().map_err(SecurityResonanceStoreError::ResolveRuntimeDir)?;
        Ok(Self::new(runtime_dir.join("security_resonance.db")))
    }
