...
impl SignalOutcomeStore {
    ...
    pub fn workspace_default() -> Result<Self, SignalOutcomeStoreError> {
        if let Ok(path) = std::env::var("EXCEL_SKILL_SIGNAL_OUTCOME_DB") {
            return Ok(Self::new(PathBuf::from(path)));
        }

        let runtime_dir =
            workspace_runtime_dir().map_err(SignalOutcomeStoreError::ResolveRuntimeDir)?;
        Ok(Self::new(runtime_dir.join("signal_outcome_research.db")))
    }
