...
fn security_decision_submit_approval_writes_runtime_files_for_ready_case() {
    ...
    // 2026-04-09 CST: 这里改为校验方向链路一致性，原因是主链已把 recommendation_action / exposure_side 与旧 direction 解耦；
    // 目的：避免继续把旧的 Long 语义写死，同时仍保证 position_plan 与决策对象保持同一正式方向。 [2026-04-09 CST]
    assert_eq!(
        output["data"]["position_plan"]["plan_direction"],
        output["data"]["approval_request"]["position_plan_binding"]["plan_direction"]
    );
    assert_eq!(
...
fn security_decision_submit_approval_maps_blocked_status_and_auto_reject_flags() {
    ...
    // 2026-04-09 CST: 这里改为断言持久化方向与输出方向一致，原因是 blocked 主链现在可能落成 NoTrade 而不是旧 Long；
    // 目的：继续锁定治理链的一致性，而不是把已过时的旧方向枚举硬编码进测试。 [2026-04-09 CST]
    assert_eq!(
        persisted_decision["direction"],
        output["data"]["position_plan"]["plan_direction"]
    );
