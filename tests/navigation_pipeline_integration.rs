...
fn navigation_pipeline_filters_route_by_concept_metadata_in_scope() {
    ...
    // 2026-04-09 CST: 这里修正 hits 顺序断言，原因是 retrieval 在文本分数与来源层级相同后还会继续比较
    // locator 精度；当前 revenue 证据范围比 gross_margin 更小，因此应优先返回。
    // 目的：让集成测试跟随 retrieval 的真实排序合同，而不是误把 route 顺序投射到 hits 顺序。
    assert_eq!(result.hits[0].node_id, "node-revenue-1");
    assert_eq!(result.hits[1].node_id, "node-gross-margin-1");
}
...
fn navigation_pipeline_keeps_tag_filtered_intersection_seeds_in_route_order() {
    ...
    assert_eq!(result.roaming_path[1].from_concept_id, "revenue");
    assert_eq!(result.hits.len(), 2);
    // 2026-04-09 CST: 这里修正复合集成样本的 hits 顺序，原因是两个节点文本分数持平时，
    // retrieval 会继续按 locator 精度排序，而 revenue 的 `A1:B12` 比 gross_margin 的 `B2:C20` 更具体。
    // 目的：固定“route 保序”和“hits 由 retrieval 规则排序”这两个层次的边界。
    assert_eq!(result.hits[0].node_id, "node-revenue-1");
    assert_eq!(result.hits[1].node_id, "node-gross-margin-1");
    assert_eq!(
        result.citations,
...
fn navigation_pipeline_keeps_multi_tag_intersection_seeds_and_evidence() {
    ...
    assert_eq!(result.hits.len(), 2);
    // 2026-04-09 CST: 这里修正多标签样本的 hits 顺序，原因是该样本同样会在同分后落到 locator 精度 tie-break，
    // 因而更具体的 revenue 证据会排在 gross_margin 之前。
    // 目的：避免测试继续把 hits 排序误判为 route 排序，稳定表达 pipeline 与 retrieval 的职责分层。
    assert_eq!(result.hits[0].node_id, "node-revenue-1");
    assert_eq!(result.hits[1].node_id, "node-gross-margin-1");
    assert_eq!(
        result.citations,
