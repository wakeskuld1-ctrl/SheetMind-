...
fn create_stock_history_csv(prefix: &str, file_name: &str, rows: &[String]) -> PathBuf {
    ...
    let fixture_dir = PathBuf::from("tests")
        ...
        .join("security_scorecard")
        .join(format!("{prefix}_{unique_suffix}"));
    fs::create_dir_all(&fixture_dir).expect("security scorecard fixture dir should exist");
...
fn submit_approval_persists_formal_scorecard_even_without_model_artifact() {
    ...
    // 2026-04-09 CST: 这里改为现场构造 runtime DB，原因是原测试依赖未跟踪的 live SQLite，clean worktree 后会直接退化成“没有历史数据”；
    // 目的：把 scorecard 正式对象回归变成仓库内可复现测试，避免后续 AI 再把 local_memory 脏产物误当成正式夹具。 [2026-04-09 CST]
    let runtime_db_path = create_test_runtime_db("security_scorecard_submit_approval");
    let stock_csv = create_stock_history_csv(
        "security_scorecard_submit_approval",
        "stock.csv",
