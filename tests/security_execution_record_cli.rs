...
#[test]
fn security_execution_record_condition_review_binding_is_persisted() {
    let runtime_db_path = create_test_runtime_db("security_execution_record_condition_review");
    let server = prepare_security_environment(
        &runtime_db_path,
        "security_execution_record_condition_review",
    );
    let mut request = execution_request();
    // 2026-04-10 CST: 这里补 execution_record 挂接 condition_review 的红测，原因是 Task 5 要把投中复核正式接到执行层；
    // 目的：锁定 execution_record 至少能稳定沉淀复核 ref、动作和摘要，避免后续只剩口头解释无法复盘。
    ...
    assert_eq!(output["status"], "ok");
    assert_eq!(
        output["data"]["execution_record"]["condition_review_ref"],
        "condition-review:601916.SH:2025-09-17:manual_review:v1"
    );
    assert_eq!(
        output["data"]["execution_record"]["condition_review_follow_up_action"],
        "keep_plan"
    );
    assert_eq!(
        output["data"]["execution_record"]["condition_review_trigger_type"],
        "manual_review"
    );
    assert!(
        output["data"]["execution_record"]["condition_review_summary"]
            .as_str()
            .expect("condition review summary should exist")
            .contains("keep_plan")
    );
}
...
fn condition_review_document_fixture() -> Value {
    json!({
