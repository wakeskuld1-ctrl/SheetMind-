...
fn security_scorecard_training_generates_artifact_and_registers_refit_outputs() {
    ...
    // 2026-04-09 CST: 这里调缓负样本的下跌斜率并抬高起点，原因是旧参数会在长窗口里过早贴到价格下限，
    // 目的：继续保留稳定负标签样本，同时避免 RSRS 低点序列退化成常量导致分母为 0。 [2026-04-09 CST]
    fs::write(&stock_down_csv, build_trend_rows(420, 260.0, -0.35, 1.0).join("\n"))
