...
pub fn thread_result_ref_store() -> ResultRefStore {
    ResultRefStore::new(thread_runtime_root().join("result_refs"))
}

#[allow(dead_code)]
pub fn runtime_result_ref_store(runtime_db_path: &PathBuf) -> ResultRefStore {
    let runtime_root = runtime_db_path
        .parent()
        .expect("runtime db path should always have a parent directory");
