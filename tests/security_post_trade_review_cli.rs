...
#[test]
fn security_post_trade_review_reads_condition_review_and_explains_follow_up() {
    let runtime_db_path = create_test_runtime_db("security_post_trade_review_condition_review");
    let server = prepare_security_environment(
        &runtime_db_path,
        "security_post_trade_review_condition_review",
    );
    let mut request = review_request();
    // 2026-04-10 CST: 这里补 post_trade_review 读取 condition_review 的红测，原因是 Task 5 要让投后层能解释最近一次投中复核；
    // 目的：锁定 review 文档对复核引用、动作和解释语句的正式输出，避免后续复盘只能回看 execution 层原始字段。
    request["args"]["condition_review_ref"] =
        json!("condition-review:601916.SH:2025-09-17:manual_review:v1");
    request["args"]["condition_review_document"] = condition_review_document_fixture();
    ...
    assert_eq!(output["status"], "ok");
    assert_eq!(
        output["data"]["post_trade_review"]["condition_review_ref"],
        "condition-review:601916.SH:2025-09-17:manual_review:v1"
    );
    assert_eq!(
        output["data"]["post_trade_review"]["condition_review_follow_up_action"],
        "keep_plan"
    );
    assert_eq!(
        output["data"]["post_trade_review"]["condition_review_trigger_type"],
        "manual_review"
    );
    assert!(
        output["data"]["post_trade_review"]["condition_review_interpretation"]
            .as_str()
            .expect("condition review interpretation should exist")
            .contains("keep_plan")
    );
}
...
fn condition_review_document_fixture() -> serde_json::Value {
    json!({
        "condition_review_id": "condition-review:601916.SH:2025-09-17:manual_review:v1",
        "contract_version": "security_condition_review.v1",
        "generated_at": "2026-04-10T09:30:00+08:00",
        "symbol": "601916.SH",
        "analysis_date": "2025-09-17",
        "decision_ref": "decision:601916.SH:2025-09-17:v1",
        "approval_ref": "approval:601916.SH:2025-09-17:v1",
        "position_plan_ref": "position-plan:601916.SH:2025-09-17:v1",
        "package_path": "artifacts/decision_packages/601916.SH-2025-09-17.json",
        ...
        "review_findings": [
            ...
            "follow_up_action=keep_plan",
            "盘中人工复核：检查原有持仓计划是否仍成立"
        ],
        "review_summary": "601916.SH 在 manual_review 触发下完成条件复核，当前建议动作为 keep_plan。"
    })
}
