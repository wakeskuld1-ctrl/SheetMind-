...
fn write_foundation_fixture_json(file_name: &str, payload: &serde_json::Value) -> PathBuf {
    // 2026-04-12 CST: Added a dedicated foundation JSON fixture writer because
    // the new repository-metadata-audit export tool is file-driven and the CLI
    // contract test must build stable schema/bundle inputs on disk. Purpose:
    // keep the test focused on the tool contract instead of inline temp-file
    // handling noise in each assertion.
    let fixture_dir = thread_runtime_root().join("foundation_cli_fixtures");
...
fn write_foundation_fixture_text(file_name: &str, payload: &str) -> PathBuf {
    // 2026-04-12 CST: Added a plain-text fixture writer because the navigation
    // export public tool now needs JSONL contract coverage in addition to JSON.
    // Purpose: keep line-oriented bundle fixtures reusable and out of the test
    // body so the assertions stay focused on behavior.
    let fixture_dir = thread_runtime_root().join("foundation_cli_fixtures");
    fs::create_dir_all(&fixture_dir).unwrap();
...
fn create_repository_metadata_audit_schema_fixture() -> PathBuf {
    // 2026-04-12 CST: Added a minimal metadata schema fixture because the export
    // tool contract should prove it can load a file-backed schema instead of
    // consuming in-memory constructors from test code. Purpose: lock the public
    // CLI entry onto the same file contract future AI flows will use.
    write_foundation_fixture_json(
        "repository_metadata_audit_schema.json",
        &json!({
            ...
            "concept_policies": [
                {
                    "concept_id": "sheet_fact",
                    "allowed_field_keys": ["owner"],
                    "required_field_keys": []
                }
            ]
        }),
...
fn create_repository_metadata_audit_bundle_fixture() -> PathBuf {
    // 2026-04-12 CST: Added a minimal repository bundle fixture because the
    // export tool contract should return DTO JSON from a persisted foundation
    // bundle, not a hand-built in-memory repository. Purpose: keep the test on
    // the real bundle-file ingestion path used by the dispatcher.
    write_foundation_fixture_json(
        "repository_metadata_audit_bundle.json",
        &json!({
            ...
            "nodes": [
                {
                    "id": "node_001",
                    "title": "Revenue Row",
                    "body": "Revenue fact extracted from workbook",
                    "concept_ids": ["sheet_fact"],
                    "evidence_refs": [],
                    "metadata": {
                        "owner": "foundation"
                    }
...
fn create_repository_metadata_audit_bundle_jsonl_fixture() -> PathBuf {
    // 2026-04-13 CST: Added a JSONL repository-audit bundle fixture because
    // the public foundation export boundary should accept the same persisted
    // repository formats as the navigation export tool. Purpose: keep both
    // public foundation outlets aligned on bundle-file ingestion semantics.
    write_foundation_fixture_text(
        "repository_metadata_audit_bundle.jsonl",
        r#"{"record_type":"bundle_header","schema_version":"knowledge-bundle:v1"}
{"record_type":"concept","id":"sheet_fact","name":"Sheet Fact","aliases":[]}
{"record_type":"node","id":"node_001","title":"Revenue Row","body":"Revenue fact extracted from workbook","concept_ids":["sheet_fact"],"metadata":{"owner":"foundation"},"evidence_refs":[]}
"#,
    )
}

fn create_navigation_export_bundle_fixture() -> PathBuf {
    // 2026-04-12 CST: Added a minimal navigation bundle fixture because the
    // new CLI export tool must prove it can rebuild the full foundation
    // roaming mainline from a persisted knowledge bundle. Purpose: keep the
    // contract test on the same file-driven repository path future AI callers
    // will use instead of assembling stores manually inside the test.
    write_foundation_fixture_json(
        "navigation_export_bundle.json",
        &json!({
            "schema_version": "knowledge-bundle:v1",
            "concepts": [
                {
            ...
            "relations": [
                {
                    "from_concept_id": "revenue",
                    "to_concept_id": "invoice",
            ...
            "nodes": [
                {
                    "id": "node_revenue_001",
                    "title": "Revenue Trend",
                    "body": "Revenue trend evidence from workbook.",
                    "concept_ids": ["revenue"],
                    "evidence_refs": [
                        {
...
fn create_navigation_export_bundle_jsonl_fixture() -> PathBuf {
    // 2026-04-12 CST: Added a JSONL navigation bundle fixture because the new
    // public export tool accepts both JSON and JSONL repository inputs.
    // Purpose: lock the shared dispatcher boundary across both persisted bundle
    // formats before higher-level AI flows depend on it.
    write_foundation_fixture_text(
        "navigation_export_bundle.jsonl",
        r#"{"record_type":"bundle_header","schema_version":"knowledge-bundle:v1"}
...
{"record_type":"node","id":"node_revenue_001","title":"Revenue Trend","body":"Revenue trend evidence from workbook.","concept_ids":["revenue"],"metadata":{"source":"sheet:sales"},"evidence_refs":[{"source_ref":"sheet:sales","locator":"A1:B12"}]}
"#,
    )
}
...
#[test]
fn tool_catalog_includes_repository_metadata_audit_export_v1() {
    let mut cmd = Command::cargo_bin("excel_skill").unwrap();
    let assert = cmd.assert().success();
    let output = String::from_utf8(assert.get_output().stdout.clone()).unwrap();
    let json: serde_json::Value = serde_json::from_str(&output).unwrap();

    // 2026-04-12 CST: Added a catalog contract for the foundation audit export
    // tool because downstream AI should discover the DTO-facing outlet from the
    // public tool list instead of relying on hidden dispatcher knowledge.
    assert!(
        json["data"]["tool_catalog"]
            .as_array()
            .unwrap()
            .iter()
            .any(|tool| tool == "repository_metadata_audit_export_v1")
    );
}

#[test]
fn repository_metadata_audit_export_v1_returns_dto_json_from_cli() {
    let schema_path = create_repository_metadata_audit_schema_fixture();
    let bundle_path = create_repository_metadata_audit_bundle_fixture();
    let request = json!({
        "tool": "repository_metadata_audit_export_v1",
        "args": {
    ...
    assert_eq!(output["status"], "ok");
    // 2026-04-12 CST: Added a DTO contract assertion because this export tool is
    // the first stable external outlet for repository metadata audit results.
    // Purpose: lock the top-level shape and a few key governance fields before
    // wiring more AI flows onto the DTO surface.
    assert_eq!(output["data"]["total_nodes"], 1);
    assert_eq!(output["data"]["audited_nodes"], 1);
    assert_eq!(output["data"]["hygiene_summary"]["total_diagnostics"], 1);
    assert_eq!(
        output["data"]["hygiene_views"]["by_severity"][0]["severity"],
        "Critical"
    );
    assert_eq!(
        output["data"]["hygiene_diagnostics"][0]["diagnostic_type"],
        "MissingEvidenceRef"
    );
}

#[test]
fn repository_metadata_audit_export_v1_accepts_jsonl_bundle_input() {
    let schema_path = create_repository_metadata_audit_schema_fixture();
    let bundle_path = create_repository_metadata_audit_bundle_jsonl_fixture();
    let request = json!({
        "tool": "repository_metadata_audit_export_v1",
        "args": {
            "schema_path": schema_path.to_string_lossy(),
    ...
    // 2026-04-13 CST: Added a RED contract for JSONL audit-export input
    // because the public foundation export tools should stay aligned on
    // persisted repository formats. Purpose: prevent one export path from
    // drifting back to an accidental JSON-only loader.
    assert_eq!(output["status"], "ok");
    assert_eq!(output["data"]["total_nodes"], 1);
    assert_eq!(
        output["data"]["hygiene_diagnostics"][0]["diagnostic_type"],
...
#[test]
fn repository_metadata_audit_export_v1_rejects_blank_schema_path() {
    let bundle_path = create_repository_metadata_audit_bundle_fixture();
    let request = json!({
        "tool": "repository_metadata_audit_export_v1",
        "args": {
    ...
    // 2026-04-13 CST: Added a RED contract for blank schema paths because the
    // audit export tool should fail at the dispatcher boundary before touching
    // filesystem loading. Purpose: align public bad-input semantics across the
    // foundation export tools.
    assert_eq!(output["status"], "error");
    assert_eq!(
        output["error"],
        "repository_metadata_audit_export_v1 missing schema_path"
    );
}

#[test]
fn repository_metadata_audit_export_v1_rejects_blank_bundle_path() {
    let schema_path = create_repository_metadata_audit_schema_fixture();
    let request = json!({
        "tool": "repository_metadata_audit_export_v1",
        "args": {
    ...
    // 2026-04-13 CST: Added a RED contract for blank bundle paths because the
    // audit export tool should reject missing persisted-repository input before
    // loader selection. Purpose: keep public export-tool boundary rules
    // consistent inside foundation.
    assert_eq!(output["status"], "error");
    assert_eq!(
        output["error"],
        "repository_metadata_audit_export_v1 missing bundle_path"
    );
}

#[test]
fn tool_catalog_includes_navigation_evidence_export_v1() {
    let mut cmd = Command::cargo_bin("excel_skill").unwrap();
    let assert = cmd.assert().success();
    let output = String::from_utf8(assert.get_output().stdout.clone()).unwrap();
    let json: serde_json::Value = serde_json::from_str(&output).unwrap();

    // 2026-04-12 CST: Added a catalog contract for the navigation export tool
    // because downstream AI and CLI flows should discover the stable DTO
    // outlet from the public tool list. Purpose: prevent hidden-only use of
    // the new roaming export boundary.
    assert!(
        json["data"]["tool_catalog"]
            .as_array()
            .unwrap()
            .iter()
            .any(|tool| tool == "navigation_evidence_export_v1")
    );
}

#[test]
fn navigation_evidence_export_v1_returns_navigation_dto_json_from_cli() {
    let bundle_path = create_navigation_export_bundle_fixture();
    let request = json!({
        "tool": "navigation_evidence_export_v1",
        "args": {
            "bundle_path": bundle_path.to_string_lossy(),
            "question": "show sales trend",
            "seed_concept_ids": ["revenue"],
    ...
    // 2026-04-12 CST: Added a DTO export contract because the stable
    // NavigationEvidenceExportDtoV1 surface must be reachable from the public
...
#[test]
fn navigation_evidence_export_v1_applies_metadata_constraints_from_cli() {
    let bundle_path = create_navigation_export_bundle_fixture();
    let request = json!({
        "tool": "navigation_evidence_export_v1",
        "args": {
    ...
    // 2026-04-12 CST: Added a public metadata-aware export contract because
    // higher-level callers should be able to pass stable metadata filters on
    // the public tool boundary instead of reaching into internal request
    // structs. Purpose: keep metadata scope on the same stable export path.
    assert_eq!(output["status"], "ok");
    assert_eq!(output["data"]["hits"][0]["node_id"], "node_revenue_001");
    assert_eq!(
        output["data"]["citations"],
...
#[test]
fn navigation_evidence_export_v1_surfaces_error_when_metadata_constraints_filter_all_hits() {
    let bundle_path = create_navigation_export_bundle_fixture();
    let request = json!({
        "tool": "navigation_evidence_export_v1",
        "args": {
    ...
    // 2026-04-12 CST: Added a failure-path contract because metadata-aware
    // public export is only real if an excluding filter can change the outcome.
    // Purpose: prove the dispatcher passes metadata constraints into the
    // foundation mainline instead of silently ignoring them.
    assert_eq!(output["status"], "error");
    assert!(
        output["error"]
            .as_str()
            .unwrap()
            .contains("RetrievalFailed")
    );
}

#[test]
fn navigation_evidence_export_v1_accepts_jsonl_bundle_input() {
    let bundle_path = create_navigation_export_bundle_jsonl_fixture();
    let request = json!({
        "tool": "navigation_evidence_export_v1",
        "args": {
            "bundle_path": bundle_path.to_string_lossy(),
    ...
    // 2026-04-12 CST: Added a JSONL contract because the public roaming export
    // boundary should accept the same persisted repository formats as the
...
#[test]
fn navigation_evidence_export_v1_rejects_unsupported_metadata_operator() {
    let bundle_path = create_navigation_export_bundle_fixture();
    let request = json!({
        "tool": "navigation_evidence_export_v1",
        "args": {
    ...
    // 2026-04-13 CST: Added a RED contract for unsupported metadata operators
    // because the public dispatcher boundary should fail with a stable and
    // caller-readable message before the request reaches internal foundation
    // logic. Purpose: stop future AI callers from relying on accidental
    // operator fallthrough semantics.
    assert_eq!(output["status"], "error");
    assert_eq!(
        output["error"],
        "navigation_evidence_export_v1 metadata_constraints operator 不支持: prefix"
    );
}

#[test]
fn navigation_evidence_export_v1_rejects_equals_without_value() {
    let bundle_path = create_navigation_export_bundle_fixture();
    let request = json!({
        "tool": "navigation_evidence_export_v1",
        "args": {
    ...
    // 2026-04-13 CST: Added a RED contract for missing equals values because
    // the public metadata JSON shape should reject incomplete single-value
    // filters explicitly. Purpose: keep v1 callers from guessing whether a
    // missing value means wildcard or silent empty-string coercion.
    assert_eq!(output["status"], "error");
    assert_eq!(
        output["error"],
        "navigation_evidence_export_v1 metadata_constraints 缺少 value: source"
    );
}

#[test]
fn navigation_evidence_export_v1_rejects_set_operators_without_values() {
    let bundle_path = create_navigation_export_bundle_fixture();
    let request = json!({
        "tool": "navigation_evidence_export_v1",
        "args": {
    ...
    // 2026-04-13 CST: Added a RED contract for empty set-style metadata
    // filters because `in` and `has_any` should fail fast instead of silently
    // becoming impossible matches. Purpose: stabilize the public boundary for
    // AI callers that build JSON requests programmatically.
    assert_eq!(output["status"], "error");
    assert_eq!(
        output["error"],
        "navigation_evidence_export_v1 metadata_constraints 缺少 values: channel"
    );
}

#[test]
fn navigation_evidence_export_v1_rejects_range_without_bounds() {
    let bundle_path = create_navigation_export_bundle_fixture();
    let request = json!({
        "tool": "navigation_evidence_export_v1",
        "args": {
    ...
    // 2026-04-13 CST: Added a RED contract for empty range bounds because the
    // public dispatcher should not accept a no-op range filter. Purpose: keep
    // future contract evolution explicit instead of relying on ambiguous empty
    // range semantics.
    assert_eq!(output["status"], "error");
    assert_eq!(
        output["error"],
        "navigation_evidence_export_v1 metadata_constraints range 缺少 min/max: observed_at"
    );
}

#[test]
fn navigation_evidence_export_v1_uses_route_only_defaults_when_optional_plan_args_are_omitted() {
    let bundle_path = create_navigation_export_bundle_fixture();
    let request = json!({
        "tool": "navigation_evidence_export_v1",
        "args": {
    ...
    // 2026-04-13 CST: Added a RED contract for omitted optional plan args
    // because the public DTO outlet should remain usable even when callers
...
#[test]
fn navigation_evidence_export_v1_rejects_blank_question() {
    let bundle_path = create_navigation_export_bundle_fixture();
    let request = json!({
        "tool": "navigation_evidence_export_v1",
        "args": {
    ...
    // 2026-04-13 CST: Added a RED contract for blank questions because the
    // public boundary should reject missing semantic input before routing.
...
#[test]
fn navigation_evidence_export_v1_rejects_blank_bundle_path() {
    let request = json!({
        "tool": "navigation_evidence_export_v1",
        "args": {
            "bundle_path": "   ",
    ...
    // 2026-04-13 CST: Added a RED contract for blank bundle paths because the
    // public dispatcher should fail before touching filesystem loaders when the
...
#[test]
fn navigation_evidence_export_v1_rejects_blank_metadata_field() {
    let bundle_path = create_navigation_export_bundle_fixture();
    let request = json!({
        "tool": "navigation_evidence_export_v1",
        "args": {
    ...
    // 2026-04-13 CST: Added a RED contract for blank metadata field names
    // because malformed public filters should stop at the dispatcher boundary.
    // Purpose: avoid pushing empty-field ambiguity into later foundation
    // registry or retrieval logic.
    assert_eq!(output["status"], "error");
    assert_eq!(
        output["error"],
        "navigation_evidence_export_v1 metadata_constraints 缺少 field"
    );
}
