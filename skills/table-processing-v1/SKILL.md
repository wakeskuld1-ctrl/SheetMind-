...
- `step_n_result` 目前主要用于计划说明
- 当前 Tool 层还没有把它做成完整可跨调用复用的中间结果句柄
- 所以 V1 多表能力重点是“规划和解释”，不是“任意多步自动执行到底”
...
- 要明确说明这是计划引用
- 不要假装当前 Tool 层已经支持完整跨请求链式执行
- 可以继续向用户解释计划，但不能伪造执行结果
...
## 2026-03-23 ????
- ???????????????? Sheet????????????????????
- ?? `open_workbook` ? `list_sheets` ??? `file_ref` ? `sheets`???????? `file_ref + sheet_index`?
- ????? ASCII ??????????????????????????????????
- ????????????? Tool ?????????????????/??????????????????
