...
### Goal
- 把现有“计算器”Excel 升级成稳交付版双文件方案：
  - 第一页保留现场输入、目标输入和测算按钮
  - 第二页展示 Rust 求出的最优调整建议
  - 交付件形态为 `xlsm + solver.exe`

### Scope
- 锁定第一页/第二页的页面契约与字段边界
- 为“只能下调、整数约束、最大亏损受控、退款最少、亏损号码数接近目标值”建立正式求解模型
- 设计 Rust 求解器与 Excel 薄宏按钮的协作方式
- 明确 workbook 回写路径、测试口径与最终打包方式

### Current Status
- [x] 已拆清现有 Excel 的测算逻辑
- [x] 已与用户确认优化目标、整数约束与两页式页面契约
- [x] 已批准稳交付版架构：`xlsm + Rust solver.exe`
- [x] 已完成设计收敛并开始写正式实施计划
- [ ] 尚未开始任何实现代码改动

### Acceptance Criteria
- [ ] 第一页保留当前业务输入布局，并增加目标输入区与测算按钮
- [ ] 第二页输出优化摘要、说明文字与调整明细
- [ ] 调整后下注额全部为整数且不高于原下注额
- [ ] 最大亏损值不超过目标阈值
- [ ] 在所有可行解中，总退款金额最少
- [ ] 在同等最优解中，亏损号码数量最接近目标值
- [ ] 交付件可按 `xlsm + solver.exe` 形式运行

### Phases
1. 冻结 workbook 契约与样例夹具
2. 锁定当前测算数学规则
3. 技术刺探 xlsm 回写/宏保留路径
4. 落地整数优化求解器
5. 打通第一页输入到第二页建议输出
6. 完成打包说明、验证与交接
...
|-------|---------|------------|
| 早期把问题误判为普通盈亏展示表 | 1 | 已改为“受约束整数优化问题”统一处理 |
| 早期把目标输入区放到第二页 | 1 | 已按用户要求改为第一页承载业务操作和目标输入 |
...
### Goal
- 以远端 `StockMind/main` 为唯一主流程，在独立新工程中承接后续开发，并盘点本地 `Excel_Skill` 里哪些 Rust 能力值得迁移。

### Scope
- 新建独立工程目录 `D:\Rust\StockMind`
- 拉取 `https://github.com/wakeskuld1-ctrl/StockMind.git`
- 创建整合分支
- 对比新旧工程关键能力模块，输出首轮迁移清单

### Current Status
- [x] 已确认远端默认分支为 `main`
- [x] 已确认原计划目录 `D:\Rust\StockMind` 会触发 Windows 路径过长问题，并已改用更短本地路径
- [x] 已落设计文档与实施计划
- [x] 已在 `D:\SM` 成功克隆新工程
- [x] 已创建整合分支 `codex/reconcile-local-features-20260417`
- [x] 已完成首轮关键能力盘点
- [ ] 待确认第一批实际迁移范围
