...
## 2026-04-10（Validator Linkage 补充）

<!-- 2026-04-10 CST：追加 validator linkage 进度记录。原因：代码和测试已经完成，但 progress 仍停留在“下一阶段方案入库”。目的：把方案 A 已落地、已验证、下一步转入方案 B 的状态补齐。 -->

- 已按用户批准的方案 A 完成 foundation `MetadataValidator` 联动，范围收口为“节点级 metadata 治理信号消费”，不扩展到 repository audit 或 migration executor。
- 已在 `tests/metadata_validator_unit.rs` 新增 3 条红测，分别钉住：
  - alias 被 canonical policy 接受但仍输出 `AliasFieldUsed`
  - deprecated 字段输出 `DeprecatedFieldUsed` 与 `replaced_by`
  - alias / deprecated 信号可同时出现且不再误报 required 缺失
- 已在 `src/ops/foundation/metadata_schema.rs` 补齐 alias 索引与 canonical 解析辅助：
  - `field_definition_for_input(...)`
  - `canonical_field_key(...)`
  - `alias_target_key(...)`
- 已在 `src/ops/foundation/metadata_validator.rs` 完成 validator 正式联动：
  - 新增 `AliasFieldUsed`
  - 新增 `DeprecatedFieldUsed`
  - `validate_node(...)` 改为按 canonical 字段继续校验
  - `node_contains_required_field(...)` 已支持 alias 与 deprecated -> replaced_by 兼容语义
- 已完成验证：
...
- 当前下一步默认进入：`Repository-Level Audit`
- 当前明确未进入：`Migration Executor`

## 2026-04-10（Repository-Level Audit 补充）

<!-- 2026-04-10 CST：追加 repository-level audit 进度记录。原因：方案B已按 TDD 落地，需要把“仓库级聚合审计已完成”正式补进 progress。目的：固定本轮新增模块、测试和边界结论。 -->

- 已按用户批准的方案 B 完成 foundation `Repository-Level Audit`，范围收口为“只读聚合审计 + 最小 evidence hygiene diagnostics”，不扩展到自动迁移。
- 已新增 `tests/repository_metadata_audit_unit.rs` 红测，钉住：
  - repository 全节点 issue 聚合
  - `issue_type_counts`
  - `concept_issue_counts`
  - `DuplicateEvidenceRef / WeakLocator / WeakSourceRef`
- 已新增 `src/ops/foundation/repository_metadata_audit.rs` 最小实现，并在 `src/ops/foundation.rs` 正式导出模块。
- 已完成验证：
...
- 当前下一步默认进入：扩细 `evidence hygiene diagnostics`
- 当前明确未进入：`Migration Executor`

## 2026-04-10 Progress: Foundation Evidence Hygiene Diagnostics Expansion
- RED：`repository_metadata_audit_unit` 新增断言后失败，缺少 `MissingEvidenceRef`、`DuplicateEvidenceRefWithinNode` 与弱诊断原因枚举。
- GREEN：在 `src/ops/foundation/repository_metadata_audit.rs` 补齐枚举、诊断变体和只读收集逻辑。
- 回归：foundation 相关测试集通过；现存 `dead_code` warnings 来自既有 `src/tools/dispatcher.rs`，不属于本轮引入。
- 能力累计：repository-level evidence hygiene 从 3 项提升到 7 项。
